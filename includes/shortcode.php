...
function all_trips_shortcode($atts) {
    ...
    if (!empty($atts['design'])) {
        ...
        if ($design === null && isset($designs[$design_id])) {
        ...
        if ($design) {
            ...
            if (!empty($design['displayType'])) $block_atts['displayType'] = $design['displayType'];
            if (!empty($design['buttonType'])) $block_atts['buttonType'] = $design['buttonType'];
            if (!empty($design['buttonText'])) $block_atts['buttonText'] = $design['buttonText'];
            if (!empty($design['buttonColor'])) $block_atts['buttonColor'] = $design['buttonColor'];
            if (!empty($design['tripType'])) $block_atts['tripType'] = $design['tripType'];

            // Handle date range
            if (!empty($design['dateRangeStart'])) $block_atts['dateStart'] = $design['dateRangeStart'];
            if (!empty($design['dateRangeEnd'])) $block_atts['dateEnd'] = $design['dateRangeEnd'];
        }
    }
...
function render_all_trips_fallback($atts) {
    ...
    if (empty($atts['buttonText'])) {
        $atts['buttonText'] = $atts['buttonType'] === 'book_now' ? 'Book Now' : 'View Trip';
    }
    ...
    <div class="wp-block-all-trips-block">
        ...
        <?php if ($atts['displayType'] !== 'carousel'): ?>
